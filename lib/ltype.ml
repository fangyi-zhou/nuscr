...
let show =
  ...
  let rec show_local_type_internal indent =
    ...
    function
    | (RecvL (m, plist, r, l) | SendL (m, plist, r, l)) as t ->
        let phantom =
          match plist with
          | [] -> ""
          | plist ->
              sprintf "[%s]"
                (String.concat ~sep:", " (List.map ~f:show_payload plist))
        in
        let direction =
          match t with
          | RecvL _ -> "from"
          ...
          | _ -> assert false
        in
        sprintf "%s%s%s %s %s;\n%s" current_indent (show_message m) phantom
          direction (RoleName.user r)
...
let rec merge projected_role lty1 lty2 =
  try
    ...
    | RecvL (_, _, r1, _), RecvL (_, _, r2, _) ->
        if not @@ RoleName.equal r1 r2 then fail () ;
        merge_recv r1 [lty1; lty2]
    | ChoiceL (r1, ltys1), RecvL (_, _, r2, _) when RoleName.equal r1 r2 ->
        (* Choice is a set of receive *)
        merge_recv r1 (lty2 :: ltys1)
    | RecvL (_, _, r2, _), ChoiceL (r1, ltys2) when RoleName.equal r1 r2 ->
...
let rec project (projected_role : RoleName.t) =
  let rec aux projected_role plist = function
    | EndG -> EndL
    ...
    | ChoiceG (choice_r, g_types) -> (
        ...
        let l_types = List.map ~f:(aux projected_role plist) g_types in
        let l_types =
